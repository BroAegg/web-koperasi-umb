  ...
  return withDeveloperSession(req, async () => {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      for (const item of items) {
        ...
        // ✅ NEW: Auto-create consignment_sales for TITIPAN products
        if (product.ownershipType === 'TITIPAN' || product.isConsignment) {
          ...
          if (activeBatch) {
            ...
            // Create consignment_sales record (creates hutang konsinyasi)
            ...
            console.log('[POS] Created consignment_sales:', {
              productId: item.productId,
              batchId: activeBatch.id,
              qtySold: item.quantity,
              netToConsignor,
